...
class API:
    def __init__(self):
        self.host='10.42.0.1'; self.port=DEFAULT_PORT; self.code=''
    ...
    def call(self,path,method='GET',payload=None,timeout=4,auth=True):
        ...
        if auth and self.code:
            headers['X-PPH-Token']=self.code
    ...
    def command(self,kind):
        path=START_PATH if kind=='start' else STOP_PATH
        return self.call(path,method='POST',payload={})
...
class App:
    ...
    def command(self,kind):
        def work():
            self.configure_api(); self.api.command(kind); time.sleep(1.0); d=self.api.call(STATUS_PATH); self.root.after(0,lambda:self.apply(d,True))
        self.bg(work)
    def schedule(self):
        if self.after_id:
