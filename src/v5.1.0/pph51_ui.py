...
CYAN='#42dcff'; GREEN='#45e39a'; ORANGE='#ff9f68'; PURPLE='#bd8cff'; RED='#ff6475'; BLUE='#6f8cff'; YELLOW='#ffd166'

CORE=('home3','measure3','wifi3','network','access3','system3','settings3','hardware','storage','services','events3','jobs3','reports','tools','network_doctor','lan_mapper','field50','flow50','compare50','session50','notify50')
...
def install(cls:type, ns:dict[str,Any])->None:
    ...
    def button(self,p,text,cmd,accent=CYAN):
        b=tk.Button(p,text=text,command=cmd,bg=PANEL2,fg=TEXT,activebackground=accent,activeforeground=BG,
                    ...
                    font=self._font(12,'bold'),pady=14,padx=8,cursor='hand2')
        return b

    def header(self,p,kicker,title,status='READY'):
        h=tk.Frame(p,bg=BG,height=62); h.pack(fill='x',padx=12,pady=(7,2)); h.pack_propagate(False)
        l=tk.Frame(h,bg=BG); l.pack(side='left',fill='y')
        tk.Label(l,text=kicker,bg=BG,fg=CYAN,font=self._font(8,'bold')).pack(anchor='w')
        tk.Label(l,text=title,bg=BG,fg=TEXT,font=self._font(19,'bold')).pack(anchor='w')
        tk.Label(h,text=status,bg=PANEL2,fg=GREEN,font=self._font(9,'bold'),padx=10,pady=7).pack(side='right',pady=10)

    def nav(self,p,active):
        n=tk.Frame(p,bg='#050a10',height=78,highlightthickness=1,highlightbackground=BORDER)
        ...
        for i,(lab,target) in enumerate(items):
            ...
            b.configure(font=self._font(10,'bold'),pady=14,bg=CYAN if target==active else PANEL2,fg=BG if target==active else TEXT)
    ...
    def card(self,p,title,var,accent=CYAN,detail=None):
        ...
        tk.Label(f,textvariable=var,bg=PANEL,fg=accent,font=self._font(25,'bold')).pack(anchor='w',padx=14,pady=(2,0))
    ...
    def build_home(self):
        p=replace(self,'home3','HOME'); header(self,p,'PPH 5.1 · FIELD SYSTEM','FIELD CENTER'); nav(self,p,'home3')
        ...
        actions(self,p,[('WIRELESS ANALYZER',lambda:self.show_page('measure3'),ORANGE),('QUICK DIAG',lambda:self.show_page('network_doctor'),CYAN),('FIELD MODE',lambda:self.show_page('field50'),PURPLE)])
    ...
    def build_system(self):
        p=replace(self,'system3','SYSTEM'); header(self,p,'DEVICE','SYSTEM STATUS'); nav(self,p,'system3')
    ...
    def build_pages(self):
        ...
        build_home(self); build_wireless(self); build_network(self); build_access(self); build_system(self); build_settings(self); build_field(self)
