...
# ---------------------------------------------------------------------------
# PPH 7.0 - complete visual/motion reinvention of the 800x480 touch UI.
...
BG = '#05070A'; SURFACE = '#0D131C'; SURFACE2 = '#141C28'; BORDER = '#223247'
TEXT = '#F2F6FA'; MUTED = '#7C8FA3'
CYAN = '#3DDBFF'; GREEN = '#3EE8A0'; YELLOW = '#FFD75E'; ORANGE = '#FF9A5C'
RED = '#FF5C72'; PURPLE = '#B98CFF'; BLUE = '#6C93FF'
...
def install(cls: type, ns: dict[str, Any]) -> None:
    ...
    def build_shell(self) -> None:
        ...
        nav = tk.Frame(self.content, bg='#03050A', height=NAV_H, highlightthickness=1, highlightbackground=BORDER)
        nav.pack(side='bottom', fill='x'); nav.pack_propagate(False)
        ind = tk.Canvas(nav, bg='#03050A', width=W, height=4, highlightthickness=0)
        ind.place(relx=0, rely=0, relwidth=1, height=4)
        ...
        row = tk.Frame(nav, bg='#03050A'); row.pack(fill='both', expand=True, pady=(6, 4))
        for i, (lab, target) in enumerate(items):
            row.grid_columnconfigure(i, weight=1, uniform='nav7')
            cell = tk.Frame(row, bg='#03050A', cursor='hand2')
            cell.grid(row=0, column=i, sticky='nsew')
            c = tk.Canvas(cell, bg='#03050A', width=30, height=30, highlightthickness=0)
            c.pack(pady=(2, 0))
            glyph = NAV_GLYPHS[target]
            glyph(c, 15, 15, 11, TEXT)
            lbl = tk.Label(cell, text=lab, bg='#03050A', fg=TEXT, font=font(self, 9, 'bold'))
            lbl.pack(pady=(2, 0))
            for w in (cell, c, lbl):
    ...
    def set_active_nav(self, key):
        ...
        def frame(t):
            ...
            cx = cur * w
            pillw = w / n * 0.5
            self.pph7_nav_indicator.create_rectangle(cx - pillw / 2, 0, cx + pillw / 2, 4, fill=CYAN, outline='')
            if t >= 0.999:
    ...
    def header(self, page, kicker, title, status_text='READY', status_kind='ready', back_to=None):
        h = tk.Frame(page, bg=BG, height=HEADER_H); h.pack(fill='x', padx=16, pady=(10, 4)); h.pack_propagate(False)
        ...
        tk.Label(textcol, text=title, bg=BG, fg=TEXT, font=font(self, 18, 'bold')).pack(anchor='w')
    ...
    def holo_card(self, parent, title, value_var, accent=CYAN, detail_var=None, glow=False, icon=None, w=360, h=150):
        ...
        def build(_e=None):
            ...
            rounded_rect(c, 2, 2, cw - 2, ch - 2, 14, fill=SURFACE, outline=BORDER, width=1, tags='bg')
            c.create_rectangle(2, 2, 8, ch - 2, fill=accent, outline='', tags='bg')
            c.tag_lower('bg')
            if not state['built']:
                iy = 20
                if icon:
                ...
                vlbl = tk.Label(outer, textvariable=value_var, bg=SURFACE, fg=accent, font=font(self, 27, 'bold'), wraplength=w - 40, justify='left')
                vlbl.place(x=18, y=40)
    ...
    def action_button(self, parent, text, command, accent=CYAN, danger=False):
        ...
        def draw(pressed=False):
            ...
            rounded_rect(c, 2, 2, w - 2, h - 2, 12, fill=col, outline=BORDER, width=1)
    ...
    def build_home(self):
        page, content, svar, badge = page_shell(self, 'home3', 'PPH 7 · FIELD SYSTEM', 'FIELD CENTER', 'home3', 'READY', 'ready')
    ...
    def build_more(self):
        page, content, svar, badge = page_shell(self, 'more7', 'PPH HUB', 'ALLE KATEGORIEN', 'system3', 'READY', 'ready')
    ...
    def play_boot(self, on_done):
        ...
        radar = tk.Canvas(overlay, width=140, height=140, bg=BG, highlightthickness=0)
        ...
        tk.Label(overlay, text='PPH', bg=BG, fg=CYAN, font=font(self, 30, 'bold')).place(relx=0.5, rely=0.56, anchor='center')
        tk.Label(overlay, text='PORTABLE PERFORMANCE HUB', bg=BG, fg=MUTED, font=font(self, 10, 'bold')).place(relx=0.5, rely=0.62, anchor='center')
