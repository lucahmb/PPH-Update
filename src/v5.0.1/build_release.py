...
WORK=Path('/tmp/pph501'); PAYLOAD=WORK/'payload'
...
# Packaging hotfix: guarantee canonical file plus compatibility alias.
fieldctl=PAYLOAD/'fieldctl.py'
if not fieldctl.exists():
    raise SystemExit('fieldctl.py missing from base payload')
...
subprocess.run(['python3','-m','py_compile',str(alias)],check=True)
...
manifest={'format':1,'product':'pph-funktest','version':'5.0.1','min_version':'4.2.0','max_version':'5.0.0','channel':'stable','build_name':'PPH 5.0.1 · Fieldctl Packaging Hotfix','released':'2026-08-10','features':['PPH 5.0 Field Platform','fieldctl.py guaranteed','fielddctl.py compatibility alias','tarball content validation'],'files':files}
