...
def install(cls:type,ns:dict[str,Any])->None:
    ...
    def cleanup(self):
        for w in list(self.root.winfo_children()):
            if w is getattr(self,'content',None) or not isinstance(w,tk.Frame): continue
            remove=False
            try: remove = w.pack_info().get('side')=='bottom'
            except Exception: pass
            try:
                pl=w.place_info(); remove = remove or bool(pl and pl.get('rely') in ('1','1.0'))
            except Exception: pass
            if remove:
                try:w.destroy()
                except Exception:pass
    ...
    def nav(self,p,active):
        n=tk.Frame(p,bg='#050a10',height=62,highlightthickness=1,highlightbackground=BORDER);n.pack(side='bottom',fill='x');n.pack_propagate(False)
        for i,(lab,target) in enumerate([('HOME','home3'),('WLAN','measure3'),('NETZ','network'),('FIELD','field50'),('SYSTEM','system3')]):
            n.grid_columnconfigure(i,weight=1,uniform='n50');b=button(self,n,lab,lambda t=target:self.show_page(t),CYAN);b.configure(font=self._font(9,'bold'),pady=9,bg=CYAN if target==active else PANEL2,fg=BG if target==active else TEXT);b.grid(row=0,column=i,sticky='nsew',padx=2,pady=5)
    ...
    def build_pages(self):
        ...
        for d in (100,300,800): self.root.after(d,lambda s=self:cleanup(s))
