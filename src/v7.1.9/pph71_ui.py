...
def install(cls: type, ns: dict[str, Any]) -> None:
    ...
    def build_system(self):
        ...
        self.p7_cpuload = tk.StringVar(value='—'); self.p7_cpuload_d = tk.StringVar(value='Letzte 2s')
        self.p7_uptime = tk.StringVar(value='—'); self.p7_uptime_d = tk.StringVar(value='Seit Boot')
        ...
        two_cards(self, content, holo_card(self, content, 'CPU AUSLASTUNG', self.p7_cpuload, GREEN, self.p7_cpuload_d),
                  holo_card(self, content, 'UPTIME', self.p7_uptime, PURPLE, self.p7_uptime_d))
        ...
        def refresh():
            ...
            try:
                ...
                self.p7_ram.set('—')
            # CPU-Auslastung ohne blockierendes sleep(): Delta zur vorherigen
            # /proc/stat-Messung (die letzte refresh()-Runde, ~2s her dank
            # LIVE_PAGES-Intervall) statt eines eigenen Wartezyklus, der die UI
            # bei jedem Refresh kurz einfrieren wuerde.
            stat = run(['bash', '-lc', "grep '^cpu ' /proc/stat"])
            try:
                vals = [int(x) for x in stat.split()[1:9]]
                idle = vals[3] + vals[4]; total = sum(vals)
                prev = getattr(self, '_pph7_cpu_prev', None)
                self._pph7_cpu_prev = (idle, total)
                if prev:
                    dt = total - prev[1]; di = idle - prev[0]
                    if dt > 0:
                        pct = max(0.0, min(100.0, 100.0 * (dt - di) / dt))
                        animate_value(self, 'system3', 'cpuload', self.p7_cpuload, pct, lambda v: f'{v:.0f}%')
            except Exception:
                self.p7_cpuload.set('—')
            up = run(['bash', '-lc',
    ...
    def confirm_power(self, action):
        """Bestaetigungsdialog vor Neustart/Herunterfahren - Touch-Displays
        haben keine 'Ruecktaste', ein Fehltipp darf nicht sofort den Pi
        runterfahren."""
        is_shutdown = action == 'shutdown'
        title = 'HERUNTERFAHREN?' if is_shutdown else 'NEUSTART?'
        msg = ('Der Pi faehrt vollstaendig herunter. Ohne Netzwerkzugriff danach '
               'nur durch Stecker ziehen/erneut anschliessen wieder erreichbar.'
               if is_shutdown else
               'Der Pi startet neu. Dauert ca. 30-60 Sekunden, bis PPH wieder bereit ist.')
        def do_it():
            notify(self, 'Fahre herunter …' if is_shutdown else 'Starte neu …', 'warn')
            cmd = ['systemctl', 'poweroff'] if is_shutdown else ['systemctl', 'reboot']
            ...
            def worker():
                try:
                    ...
                    if result.returncode:
                        ...
                        self.root.after(0, lambda: notify(self, f'Fehlgeschlagen: {err}', 'error'))
                except Exception as exc:
                    self.root.after(0, lambda: notify(self, f'Fehlgeschlagen: {exc}', 'error'))
            threading.Thread(target=worker, daemon=True).start()
        overlay(self, title, msg, buttons=[
            ('ABBRECHEN', lambda: None, SURFACE2),
            ('HERUNTERFAHREN' if is_shutdown else 'NEUSTART', do_it, RED if is_shutdown else YELLOW),
        ])
    ...
    def build_settings_detail(self):
        ...
        row_label('POWER')
        r6 = tk.Frame(box, bg=SURFACE); r6.pack(fill='x', padx=10, pady=(0, 16))
        action_button(self, r6, 'NEUSTART', lambda: confirm_power(self, 'reboot'), YELLOW).pack(side='left', fill='x', expand=True, padx=3)
        action_button(self, r6, 'HERUNTERFAHREN', lambda: confirm_power(self, 'shutdown'), RED, danger=True).pack(side='left', fill='x', expand=True, padx=3)
    ...
    def ap_progress(self, action):
        """Animiertes Overlay fuer AP-Start/-Stop. Zeigt echte Statusdaten aus
        AccessPointController.status() und bleibt offen, bis der gewuenschte
        Zustand tatsaechlich erreicht ist (nicht nur nach fixer Wartezeit)."""
        is_start = action == 'start'
        dim = tk.Frame(self.root, bg='#000000')
        dim.place(relx=0, rely=0, relwidth=1, relheight=1)
        panel = tk.Frame(dim, bg=SURFACE, highlightthickness=1, highlightbackground=BORDER)
        panel.place(relx=0.5, rely=0.5, anchor='center', width=1, height=1)
        tk.Frame(panel, bg=(GREEN if is_start else YELLOW), height=5).pack(fill='x')
        tk.Label(panel, text=('ACCESS POINT STARTET' if is_start else 'ACCESS POINT STOPPT'),
                 bg=SURFACE, fg=TEXT, font=font(self, 16, 'bold')).pack(anchor='w', padx=20, pady=(14, 10))

        bar = progress_bar(self, panel, accent=(GREEN if is_start else YELLOW), height=10)
        bar.pack(fill='x', padx=20, pady=(0, 14))

        steps = [
            ('BEFEHL GESENDET', 20),
            ('SCHNITTSTELLE', 60),
            ('NETZWERK AKTIV' if is_start else 'NETZWERK GETRENNT', 100),
        ]
        rows_frame = tk.Frame(panel, bg=SURFACE); rows_frame.pack(fill='x', padx=20)
        row_labels = {}
        for key, _ in steps:
            r = tk.Frame(rows_frame, bg=SURFACE); r.pack(fill='x', pady=3)
            tk.Label(r, text=key, bg=SURFACE, fg=TEXT, font=font(self, 9, 'bold'), anchor='w').pack(side='left')
            st = tk.Label(r, text='…', bg=SURFACE, fg=MUTED, font=font(self, 9, 'bold'))
            st.pack(side='right')
            row_labels[key] = st

        detail_var = tk.StringVar(value='Sende Befehl …')
        tk.Label(panel, textvariable=detail_var, bg=SURFACE, fg=MUTED, font=font(self, 9),
                 justify='left', wraplength=440).pack(anchor='w', padx=20, pady=(10, 16))

        state = {'pct': 0, 'ceiling': 20}
        def set_pct(pct):
            state['pct'] = pct; bar._pph7_set(pct)
        def creep():
            if state['pct'] < state['ceiling']:
                set_pct(min(state['ceiling'], state['pct'] + 2))
        self._pph7_anim.loop('ap_progress:creep', 120, creep)

        def frame(t):
            wpx = int(480 * (0.9 + 0.1 * t)); hpx = int(320 * (0.9 + 0.1 * t))
            panel.place_configure(width=wpx, height=hpx)
        self._pph7_anim.tween('ap_progress:open', 180, frame, tier='REDUCED', ease='back')

        def close():
            self._pph7_anim.cancel('ap_progress:creep')
            self._pph7_anim.cancel('ap_progress:poll')
            try: dim.destroy()
            except Exception: pass

        def set_step(key, ok):
            row_labels[key].configure(text=('OK' if ok else '…'), fg=(GREEN if ok else MUTED))
            for k, target in steps:
                if k == key and ok:
                    state['ceiling'] = max(state['ceiling'], target)
                    set_pct(target)

        return {'close': close, 'set_step': set_step, 'set_detail': detail_var.set, 'steps': [k for k, _ in steps]}

    def ap_start(self):
        ap = _ensure_ap(self)
        ui = ap_progress(self, 'start')
        import threading
        def worker():
            try:
                if ap: ap.start()
            ...
            def after_call():
                if err:
                    ui['close']()
                    notify(self, f'Start fehlgeschlagen: {err}', 'error')
                    self._pph7_refresh.get('access3', lambda: None)()
                    return
                ui['set_step'](ui['steps'][0], True)
                ui['set_detail']('Warte auf aktive Verbindung …')
                _ap_progress_poll(self, ap, ui, is_start=True, attempt=0)
            try: self.root.after(0, after_call)
            except Exception: pass
        threading.Thread(target=worker, daemon=True).start()

    def ap_stop(self):
        ap = _ensure_ap(self)
        ui = ap_progress(self, 'stop')
        import threading
        def worker():
            try:
                if ap: ap.stop()
            ...
            def after_call():
                if err:
                    ...
                    return
                ui['set_step'](ui['steps'][0], True)
                ui['set_detail']('Warte auf Trennung …')
                _ap_progress_poll(self, ap, ui, is_start=False, attempt=0)
            try: self.root.after(0, after_call)
            except Exception: pass
        threading.Thread(target=worker, daemon=True).start()

    def _ap_progress_poll(self, ap, ui, is_start, attempt):
        st = {}
        try: st = ap.status() if ap else {}
        except Exception: st = {}
        active = bool(st.get('active'))
        iface = st.get('wifi_iface')

        if iface and attempt >= 1:
            ui['set_step'](ui['steps'][1], True)
            ssid = st.get('ssid', '—'); band = st.get('band', '—')
            ui['set_detail'](f"{ssid} · {band} · {iface}")

        done = active if is_start else (not active)
        if done:
            ui['set_step'](ui['steps'][2], True)
            if is_start:
                ui['set_detail'](f"AP-IP {st.get('ap_ip', '—')} · Bereit")
            else:
                ui['set_detail']('Getrennt.')
            self._pph7_anim.after('ap_progress:closehold', 500, lambda: (
                ui['close'](),
                notify(self, 'Access Point gestartet' if is_start else 'Access Point gestoppt', 'ok' if is_start else 'warn'),
            ))
            self._pph7_refresh.get('access3', lambda: None)()
            _ap_refresh_burst(self)
            return

        if attempt >= 40:  # ~20s bei 500ms Takt
            ui['close']()
            notify(self, 'Access Point braucht ungewöhnlich lange - Status prüfen', 'warn')
            self._pph7_refresh.get('access3', lambda: None)()
            return

        self._pph7_anim.after('ap_progress:poll', 500, lambda: _ap_progress_poll(self, ap, ui, is_start, attempt + 1))
