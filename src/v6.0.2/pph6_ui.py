...
# Pages that show live device/network state get their refresh() re-scheduled
# on this interval (ms) for as long as they stay visible, cancelled the
# instant the user navigates away (Anim.cancel_page). Without this, a "LIVE"
# status chip was a lie: refresh() only ever ran once, on page entry - e.g.
# the Access Point page kept showing ACTIVE for a while after STOP was
# pressed, because the underlying status only updates via a background
# monitor thread that polls once a second. Pages driven purely by user
# actions (settings, field mode hub, before/after, session recorder) and
# Network Doctor (explicit "RUN AGAIN", real ping/DNS probes - not something
# to fire unattended every second) are intentionally excluded.
LIVE_PAGES = {
    'home3': 2000, 'measure3': 1200, 'measure3_detail': 1500,
    'wifi3': 3000, 'radio_detail': 2000,
    'network': 1500, 'network_detail': 1500,
...
def install(cls: type, ns: dict[str, Any]) -> None:
    ...
    def header(self, page, kicker, title, status_text='READY', status_kind='ready', back_to=None):
        ...
        # Persistent top-bar version control: available on every page without
    ...
    def build_access_detail(self):
        ...
        def refresh():
            ...
            band = s.get('band') or '—'
            self.p6_ad['band'].set(band if active else f'{band} (inaktiv)')
    ...
    def build_access_detail2(self):
        ...
        for key, label in (('rxtx', 'RX / TX'), ('lan', 'LAN UPLINK'), ('forwarding', 'FORWARDING (NAT)'), ('internet', 'INTERNET')):
            v = tk.StringVar(value='—'); self.p6_ad2[key] = v
            row = list_row(self, rows, label, v, 'ready')
            row.pack(fill='x', pady=3)
            self.p6_ad2_row[key] = row
        ...
        def refresh():
            ...
            active = bool(s.get('active'))
            self.p6_ad2['rxtx'].set(f"↓ {float(s.get('rx_mbps') or 0):.1f} · ↑ {float(s.get('tx_mbps') or 0):.1f} Mbit/s")
            lan_ok = bool(s.get('lan_connected'))
            self.p6_ad2['lan'].set(f"{s.get('lan_iface') or '—'} · {'VERBUNDEN' if lan_ok else 'GETRENNT'}")
            fwd_ok = bool(s.get('forwarding')) or active
            self.p6_ad2['forwarding'].set('AKTIV' if fwd_ok else 'INAKTIV')
            inet_ok = bool(s.get('internet'))
            self.p6_ad2['internet'].set('ONLINE' if inet_ok else 'OFFLINE')
            for key, ok in (('lan', lan_ok), ('forwarding', fwd_ok), ('internet', inet_ok)):
                row = self.p6_ad2_row.get(key)
                if row is not None:
                    try: row._pph6_set_kind('ready' if ok else 'offline')
                    except Exception: pass
        self._pph6_refresh['access_detail2'] = refresh
    ...
    def build_access_config(self):
        ...
        tk.Label(box, text='SSID', bg=PANEL, fg=MUTED, font=font(self, 9, 'bold')).pack(anchor='w', padx=14, pady=(16, 4))
        self.p6_cfg_ssid = tk.Entry(box, bg=PANEL2, fg=TEXT, insertbackground=TEXT, relief='flat',
                                     font=font(self, 13), highlightthickness=1, highlightbackground=BORDER)
        self.p6_cfg_ssid.pack(fill='x', padx=14, ipady=8)
        tk.Label(box, text='PASSWORT (MIND. 8 ZEICHEN)', bg=PANEL, fg=MUTED, font=font(self, 9, 'bold')).pack(anchor='w', padx=14, pady=(14, 4))
        self.p6_cfg_pass = tk.Entry(box, bg=PANEL2, fg=TEXT, insertbackground=TEXT, relief='flat',
                                     font=font(self, 13), highlightthickness=1, highlightbackground=BORDER, show='•')
        self.p6_cfg_pass.pack(fill='x', padx=14, ipady=8)
        tk.Label(box, text='LEER LASSEN, UM DAS AKTUELLE PASSWORT ZU BEHALTEN', bg=PANEL, fg=MUTED, font=font(self, 7)).pack(anchor='w', padx=14, pady=(2, 0))
        tk.Label(box, text='BAND', bg=PANEL, fg=MUTED, font=font(self, 9, 'bold')).pack(anchor='w', padx=14, pady=(14, 4))
        band_row = tk.Frame(box, bg=PANEL); band_row.pack(fill='x', padx=10)
        self.p6_cfg_band = tk.StringVar(value='a')
        self.p6_cfg_band_buttons = {}
        for label, value in (('5 GHz', 'a'), ('2.4 GHz', 'bg')):
            def select(v=value):
                self.p6_cfg_band.set(v)
                for bv, b in self.p6_cfg_band_buttons.items():
                    sel = bv == self.p6_cfg_band.get()
                    b.configure(bg=(CYAN if sel else PANEL2), fg=(BG if sel else TEXT))
            b = tk.Button(band_row, text=label, command=select, bg=PANEL2, fg=TEXT,
                          activebackground=CYAN, activeforeground=BG, relief='flat', bd=0,
                          highlightthickness=1, highlightbackground=BORDER, font=font(self, 11, 'bold'), pady=12, cursor='hand2')
            b.pack(side='left', fill='x', expand=True, padx=3)
            self.p6_cfg_band_buttons[value] = b
        action_row(self, page, [('SPEICHERN', lambda: ap_save_config(self), GREEN)])

        def refresh():
            ap = _ensure_ap(self)
            cfg = getattr(ap, 'config', {}) if ap else {}
            self.p6_cfg_ssid.delete(0, 'end'); self.p6_cfg_ssid.insert(0, str(cfg.get('ssid') or 'PPH-WIFI'))
            self.p6_cfg_pass.delete(0, 'end')
            band = str(cfg.get('band') or 'a')
            self.p6_cfg_band.set(band)
            for bv, b in getattr(self, 'p6_cfg_band_buttons', {}).items():
                sel = bv == band
                b.configure(bg=(CYAN if sel else PANEL2), fg=(BG if sel else TEXT))
        self._pph6_refresh['access_config'] = refresh
    ...
    def _ap_refresh_burst(self):
        # AccessPointController.status() is served from a background thread
        # that only re-polls nmcli once a second, so a single refresh right
        # after start()/stop() returns can still show the pre-change state.
        # Re-check a few times over the next couple of seconds so the UI
        # catches up quickly instead of waiting for whatever page-level LIVE_PAGES
        # tick happens to be due next (or forever, on a non-live page).
        for name in ('access3', 'access_detail', 'access_detail2'):
            fn = self._pph6_refresh.get(name)
            if not fn: continue
            for delay in (300, 900, 1800, 3000):
                self._pph6_anim.after(f'{name}:burst:{delay}', delay, fn)
    ...
    def ap_save_config(self):
        ap = _ensure_ap(self)
        ssid = self.p6_cfg_ssid.get().strip()
        password = self.p6_cfg_pass.get().strip()
        band = self.p6_cfg_band.get()
        if password and len(password) < 8:
            notify(self, 'Passwort muss mindestens 8 Zeichen haben.', 'error')
            return
        import threading
        def worker():
            try:
            ...
            def done():
                ...
                self._pph6_refresh.get('access3', lambda: None)()
                self._pph6_refresh.get('access_detail', lambda: None)()
                self._pph6_refresh.get('access_detail2', lambda: None)()
            try: self.root.after(0, done)
            except Exception: pass
        threading.Thread(target=worker, daemon=True).start()
    ...
    def show_page(self, name, title=None, *, push=True):
        ...
        if refresher:
            ...
            if interval:
                self._pph6_anim.loop(f'{name}:live', interval, refresher)
        try:
